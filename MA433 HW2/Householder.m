    function [xbest,fxbest,nitr,status] = Householder(f,fp,fpp,x0,epsilon,maxitr,loud)
%Housholder: zero finding using Householder's method
%
%input argument:
...
for i = 0:maxitr
    ...
    if dfunc(x) == 0
        ...
        x = x-(func(x)/dfunc(x))*(1+(func(x)*d2func(x))/(2*dfunc(x)^2));
    end
